  ...
  ({ isOpen, onSelect, onClose, position, currentPost }, forwardedRef) => {
    ...
    const commands: SlashCommand[] = [
        ...
        },
        {
          ...
          action: () => currentPost?.media?.[0]?.final_url
            ? `![](${currentPost.media[0].final_url})\n`
            : '![]()\n',
        },
        {
          id: 'link',
        ...
        },
        {
          ...
          action: () =>
            currentPost?.video?.url
              ? `<video src="${currentPost.video.url}" controls></video>\n`
    ...
    useEffect(() => {
        if (isOpen && containerRef && 'current' in containerRef && containerRef.current) {
          ...
          if (position.y + rect.height > viewportHeight) {
            container.style.top = `${position.y - rect.height}px`;
          } else {
    ...
    //   left: position.x + window.scrollX,  // Add scrollX to adjust for horizontal scrolling
    //   top: position.y + window.scrollY,   // Add scrollY to adjust for vertical scrolling
    // };
