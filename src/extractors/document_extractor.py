...
class DocumentExtractor:
    ...
    def extract_pdf(self, input_file, output_file):
        ...
        """
        try:
            # Import docling dynamically to handle potential import errors
            try:
                from docling import extract_markdown
            except ImportError:
                logger.error("Docling library not installed. Please install with: pip install docling")
                return False
            
            # Extract markdown
            markdown_content = extract_markdown(str(input_file))
