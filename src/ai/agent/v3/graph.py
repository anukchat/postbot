...
supabase: Client = utils.supabase_client()
...
class AgentWorkflow:
    ...
    def handle_feedback(self, state: BlogState):
        ...
        if "blog" in state.post_types:
            return {"final_blog": modified_content.content, "feedback_applied": True,"feedback": None}
        elif "twitter" in state.post_types:
            return {"twitter_post": modified_content.content, "feedback_applied": True,"feedback": None}
        elif "linkedin" in state.post_types:
            return {"linkedin_post": modified_content.content, "feedback_applied": True, "feedback": None}
        else:
            return state    
    ...
    def fetch_urls_and_media(self,tweet_id):
        try:
            ...
            source_response = supabase.table('sources') \
                .select('source_id') \
            ...
            urls_response = supabase.table('url_references') \
                .select('*') \
            ...
            media_response = supabase.table('media') \
                .select('*') \
                .eq('source_id', source_id) \
                .execute()
    ...
    def run_generic_workflow(self, payload):
        ...
        try:
            ...
            if payload.get("thread_id") and not payload.get("feedback"):
                ...
                content = supabase.table("content").select("*").eq("thread_id", thread_id).execute()
                if not content.data:
                    raise ValueError("No content found for thread_id")
                
                config = {"configurable": {"thread_id": thread_id}}
                self.graph.update_state(config, values={"post_types": payload.get("post_types", ["twitter", "linkedin"])})                
                ...
                for post_type in payload.get("post_types", ["twitter", "linkedin"]):
                    ...
                    existing_content = supabase.table("content").select("*").eq("thread_id", thread_id).eq("content_type_id", content_type.data[0]["content_type_id"]).execute()
                    
                    if not existing_content.data:
                        # Insert new content record for this type
                        content_body = result.get('final_blog' if post_type == "blog" else f"{post_type}_post")
                        if content_body:
                            content_data = {
            ...
            # Case 2: Feedback handling
                ...
                config = {"configurable": {"thread_id": thread_id}}
                self.graph.update_state(config, values={"feedback": payload["feedback"], "post_types": payload.get("post_types", ["blog"]),"feedback_applied": False})
    ...
    def setup_workflow(self):
        ...
        # Add routing edges
        self.builder.add_conditional_edges("generate_tags", route_after_tags, 
            ["write_linkedin_post", "write_twitter_post", "handle_feedback", END])
        
        self.builder.add_conditional_edges("write_linkedin_post", route_after_linkedin, 
            ["write_twitter_post", "handle_feedback", END])
        
        self.builder.add_conditional_edges("write_twitter_post", route_after_twitter, 
        ...
        self.builder.add_conditional_edges("handle_feedback", route_after_feedback,
        ...
        graph = self.builder.compile(
            ...
            interrupt_after=["generate_tags", "write_linkedin_post", "write_twitter_post"]
        )
        return graph
...
if __name__ == "__main__":
    ...
    # # Feedback handling
    # feedback_result = agent.run_generic_workflow({
    #     "feedback": "Remove the reference section and add more examples",
    #     "thread_id": "8f38abae-4085-41bd-bc23-022b5320313d",
    #     "post_types": ["blog"]
    # })

    # # Tweet based content
    # tweet_result = agent.run_generic_workflow({
    #     "tweet_id": "1234567890",
    #     "post_types": ["blog", "tweet"]
    # })
